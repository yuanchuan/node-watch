...
/**
 * Ignore native recursive option on MacOS to get more control
 * or temporarily set to false for opimization
 */
...
function watch(fpath, options, cb) {
  ...
  if (is.file(fpath)) {
    ...
    fs.watch(parent, noRecursive(options), function(evt, fname) {
      if (path.basename(fpath) === fname) {
        normalizeCall(fpath, noRecursive(options), cb);
      }
    }).on('error', catchException);
  } else if (is.dir(fpath)) {
    fs.watch(fpath, noRecursive(options), function(evt, fname) {
      if (fname) {
