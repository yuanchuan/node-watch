...
function makeFilter(fn, defaultValue) {
  return is.func(fn)
    ? (n) => {
        const out = fn(n);
        console.log(n, out);
...
var deprecationWarning = util.deprecate(function () {},
...
Watcher.prototype.add = function (watcher, info) {
  ...
  var internalOnChange = function (rawEvt, rawName) {
    ...
    if (info.options.recursive) {
      hasNativeRecursive(function (has) {
        var ignore = info.options.ignore;
        if (!has || ignore) {
  ...
  var handlePublicEvents = debounce(info, function (evt, name) {
    ...
    // watch directory
      ...
      if (
        info.options.filter(name) &&
        !(info.options.ignore && info.options.ignore(name))
      ) {
...
Watcher.prototype.watchFile = function (file, options, fn) {
  ...
  // TODO only set up one watcher per path
...
Watcher.prototype.watchDirectory = function (
  dir,
  options,
  fn,
  counter = nullCounter
  ...
  var done = counter();
  hasNativeRecursive(function (has) {
    const useRecurse = has && !options.ignore;
    var opts = Object.assign({}, options, {
      // always specify recursive
      recursive: useRecurse,
    ...
    if (options.recursive && !useRecurse) {
      const ignore = options.ignore;
      getSubDirectories(
        dir,
        function (d) {
          if (ignore && ignore(d)) return;
...
function watch(fpath, options, fn) {
  ...
  options.filter = makeFilter(options.filter, true);
  if (options.ignore) {
    options.ignore = makeFilter(options.ignore, false);
  }
