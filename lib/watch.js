...
var handleOptions = function(origin, defaultOptions) {
  return function() {
    ...
    args[0].forEach(function(path) {
      watchers = watchers.concat(origin.apply(null, [path].concat(args.slice(1)))); 
    });  
  ...
  if (is.file(fpath)) {
    ...
    if (options.recursive) {
      ...
      sub(fpath, function(dir) {
        watchers = watchers.concat(watch(dir, options, cb));
      });    
    }
  }
  return watchers;
