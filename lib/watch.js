...
var handleOptions = function(origin, defaultOptions) {
  return function() {
    ...
    args[0].forEach(function(path) {
      origin.apply(null, [path].concat(args.slice(1), watcher)); 
    });  
...
function watch(fpath, options, cb, watcher) {
  ...
  if (is.file(fpath)) {
    ...
    watcher.watchers.push(fs.watch(parent, function(evt, fname) {
      if (path.basename(fpath) === fname) {
        normalizeCall(fpath, options, cb, watcher);
      }
    }).on('error', catchException));                         
  } else if (is.dir(fpath)) {
    watcher.watchers.push(fs.watch(fpath, function(evt, fname) {
      if (fname) {
