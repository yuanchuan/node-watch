...
describe('watch for files', function () {
  ...
  it('should watch files inside a directory', function (done) {
    ...
    var stack = [tree.getPath('home/a/file1'), tree.getPath('home/a/file2')];
    watcher = watch(fpath, { delay: 0 }, function (evt, name) {
  ...
  it('should listen to new created files', function (done) {
    ...
    watcher.on('ready', function () {
      ...
      setTimeout(function () {
        assert.deepStrictEqual(changes, [tree.getPath(newfile1), tree.getPath(newfile2)]);
        done();
      }, 100);
    });
  });
...
describe('options', function () {
  ...
  describe('encoding', function () {
    ...
    it('should support base64 encoding', function (done) {
      ...
      watcher = watch(dir, 'base64', function (evt, name) {
        assert.equal(name, Buffer.from(fpath).toString('base64'), 'wrong base64 encoding');
        done();
      });
      watcher.on('ready', function () {
        tree.modify(file);
    ...
    it('should support hex encoding', function (done) {
      ...
      watcher = watch(dir, 'hex', function (evt, name) {
        assert.equal(name, Buffer.from(fpath).toString('hex'), 'wrong hex encoding');
        done();
      });
      watcher.on('ready', function () {
        tree.modify(file);
  ...
  describe('filter', function () {
    it('should only watch not-ignored directories', function (done) {
      ...
      watcher.on('ready', function () {
        ...
        tree.newDir('home/b/deep_node_modules');
        tree.modify('home/b/deep_node_modules/file1');
    ...
    it('should not report ignored files', function (done) {
      var dir = tree.getPath('home');
      var file1 = 'home/bb/file1';
      var file2 = 'home/bb/file2';
    ...
    it('should be able to ignore with regexp', function (done) {
      var dir = tree.getPath('home');
      var file1 = 'home/bb/file1';
      var file2 = 'home/bb/file2';
...
describe('parameters', function () {
  ...
  it('should compose array of files or directories', function (done) {
    ...
    var fpaths = [tree.getPath(file1), tree.getPath(file2)];
  ...
  it('should filter duplicate events for composed watcher', function (done) {
    ...
    var file1 = 'home/a/file1';
    var file2 = 'home/a/file2';
    var fpaths = [tree.getPath(home), tree.getPath(dir), tree.getPath(file1), tree.getPath(file2)];
    ...
    watcher.on('ready', function () {
      ...
      setTimeout(function () {
        assert.deepStrictEqual(changes, [tree.getPath(file1), tree.getPath(file2)]);
        done();
      }, 200);
    });
  });
